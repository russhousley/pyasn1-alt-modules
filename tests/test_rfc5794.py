...
"""
        ...
        for cap in asn1Object:
            ...
            if cap['parameters'].hasValue():
                ...
                param, rest = der_decoder(cap['parameters'],
                    asn1Spec=algorithmIdentifierMap[cap['capabilityID']])
                self.assertFalse(rest)
    ...
    def testOpenTypes(self):
        ...
        substrate = pem.readBase64fromText(self.pem_text)
        asn1Object, rest = der_decoder(substrate, asn1Spec=self.asn1Spec,
            openTypes=algorithmIdentifierMap, decodeOpenTypes=True)
        self.assertFalse(rest)
        self.assertTrue(asn1Object.prettyPrint())
        self.assertEqual(substrate, der_encoder(asn1Object))
        ...
        for cap in asn1Object:
            ...
            if cap['parameters'].hasValue():
                self.assertIn(cap['capabilityID'], algorithmIdentifierMap)
                if cap['capabilityID'] == rfc5794.id_aria128_ecb:       
                    self.assertEqual(rfc5794.id_pad_1, cap['parameters']['padAlgo'][0])
