...
#
# ASN.1 source from:
# https://www.rfc_editor.org/rfc/rfc9810.txt
#
...
# Updates to the Certificate Management Protocol (CMP) based on
# RFC 9480 and RFC 9810.  Where possible, import from RFC 4210.
...
# Added in RFC 9810
#
class KemBMParameter(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType('kdf', AlgorithmIdentifier()),
        namedtype.OptionalNamedType('kemContext', univ.OctetString().subtype(
...
# encryptedRand was added to the Challenge structure in RFC 9810
#
class Challenge(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.OptionalNamedType('owf', AlgorithmIdentifier()),
...
class KemCiphertextInfo(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType('ct', univ.OctetString())
    )


# Added in RFC 9810
#
class KemOtherInfo(univ.Sequence):
    componentType = namedtype.NamedTypes(
...
# Added in RFC 9810
#
id_it_KemCiphertextInfo = id_it + (24,)

class KemCiphertextInfoValue(KemCiphertextInfo):
    pass
...
_cmpInfoTypeAndValueMapUpdate = {
    ...
    id_it_KemCiphertextInfo: KemCiphertextInfoValue(),
}
